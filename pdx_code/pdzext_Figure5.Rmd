---
title: "Figure 5 binding interface"
...
#plot figure 5a: ddg&dddg&ddddg (called energy_term) in binding interface
```{r}
...
subset_dt_all <- merge(
 ...
 subset_dt_plot <- subset_dt_all[Pos %in% selected_positions]
  ...
  subset_dt_plot[, row_label := "median energy_term Binding"]
    ...
    print(p)
ggsave(paste0("Data_pdzext/output_pdzext/Fig_5a_",dataset_value,".pdf"), width=8, height=5)
}
```

#plot Figure 5b: heatmaps of interface residues
```{r}
  name <- "mutations_distances_annotation_median_Binding_merged"
    dt <- get(name)
...
p<-ggplot(subset1_dt,aes(x=dataset,y=mutant_colored,fill=total_mean_kcal_mol))+
  ...
  print(p) 
ggsave("Data_pdzext/output_pdzext/Fig_5b.pdf", width=15, height=5)
```
