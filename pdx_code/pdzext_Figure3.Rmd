...
fmt_p <- function(p, digits = 3) {
  # scientific with chosen digits; avoids "<2e-16"
  format(p, digits = digits, scientific = TRUE)
}

get_spearman_label <- function(d) {
  ct <- suppressWarnings(cor.test(d$x, abs(d$total_mean_kcal_mol),
                                 method = "spearman", exact = FALSE))
  r  <- unname(ct$estimate)
  p  <- ct$p.value
  paste0("rho = ", formatC(r, digits = 2, format = "f"),
         ", p = ", fmt_p(p, digits = 3))
}
...
for (dataset_value in dataset_values) {
  ...
  lab_all <- get_spearman_label(subset_dt)
lab_pos <- get_spearman_label(subset2_dt)
lab_neg <- get_spearman_label(subset1_dt)
  ...
  p <- ggplot() +
    ...
    # Spearman corrs only: all (black), positive (red), negative (blue)
        annotate("text",
           x = global_x_min + 0.05*(global_x_max - global_x_min),
           y = max_abs_dddg_combined*0.95,
           label = lab_all, colour = "black", size = 6, hjust = 0) +
  annotate("text",
           x = global_x_min + 0.05*(global_x_max - global_x_min),
           y = max_abs_dddg_combined*0.80,
           label = lab_pos, colour = custom_colors["positive_dddg"], size = 6, hjust = 0) +
  annotate("text",
           x = global_x_min + 0.05*(global_x_max - global_x_min),
           y = max_abs_dddg_combined*0.65,
           label = lab_neg, colour = custom_colors["negative_dddg"], size = 6, hjust = 0) +
