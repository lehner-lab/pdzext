...
for (cfg in plot_configs) {
  ...
  n_val <- sum(!is.na(data$total_mean_kcal_mol))
...
n_val <- sum(!is.na(merged_dt$total_mean_kcal_mol))

 ct <- cor.test(merged_dt$Salinas_2018_single_dG,
               merged_dt$total_mean_kcal_mol,
               method = "pearson")

lab <- paste0("r = ", signif(ct$estimate, 3),
              "\np = ", sprintf("%.3e", ct$p.value))

p <- ggplot(merged_dt, aes(x = Salinas_2018_single_dG, y = total_mean_kcal_mol)) +
  geom_smooth(method = "lm", se = FALSE, color = "lightblue", linewidth = 3) +
  geom_point(size = 3) +
  annotate("text", x = 1, y = 1, label = lab, hjust = 1.1, vjust = 1.1, size = 5) +
  coord_cartesian(ylim = global_ylim_extended) +
  theme_classic() +
  theme(aspect.ratio = 1) +
  ggtitle(paste0("Salinas2018\n n =", n_val))
  print(p)
...
#plot Supplementary Fig. 2a: violin plots core/surface/binding
```{r}
...
for (assay in assays) {
  ...
  if (assay == "Folding") {
    ...
    # ---- compute exact (numeric) t-test p-values yourself ----
  y_max <- max(subset_dt$total_mean_kcal_mol, na.rm = TRUE)

  tests_dt <- data.table::rbindlist(lapply(seq_along(comparisons_list), function(i) {
    g1 <- comparisons_list[[i]][1]
    g2 <- comparisons_list[[i]][2]

    x <- subset_dt[get(class_column) == g1, total_mean_kcal_mol]
    y <- subset_dt[get(class_column) == g2, total_mean_kcal_mol]

    tt <- t.test(x, y, paired = FALSE, alternative = "two.sided")
  ...
  # label + y positions for brackets
  tests_dt[, label := sprintf("p = %.3e", p)]
  tests_dt[, y.position := y_max * (1.08 + 0.10 * (.I - 1))]
...
ct <- cor.test(dt_merged$energy_comp, dt_merged$energy_base, method = "pearson")
    cor_lab <- paste0("r = ", signif(ct$estimate, 3),
                      "\np = ", sprintf("%.3e", ct$p.value))
  ...
  geom_vline(xintercept = 0, linetype = "dotted", color = "gray50") +
     annotate("text",
               x = x_lims[1] + 0.05 * diff(x_lims),
               y = y_lims[2] - 0.10 * diff(y_lims),
               label = cor_lab,
...
p <- ggplot(subset_dt, aes(x = dataset, y = total_mean_kcal_mol)) +
 geom_point(aes(color = color_condition), width = 0.2, alpha = 0.7, size = 5) +
  ...
  geom_line(aes(group = id), color = "black", alpha = 0.3) +
   ...
   labs(title = "Position 337", subtitle = p_value_label)+
  theme_classic() +
   theme(aspect.ratio=1)
...
p <- ggplot(subset_dt, aes(x = dataset, y = total_mean_kcal_mol)) +
  ...
  geom_line(aes(group = id), color = "black", alpha = 0.3) +
    scale_y_continuous(limits = c(-0.7,2.1)) + 
   labs(title = "Position 368", subtitle = p_value_label)+
  theme_classic() +
   theme(aspect.ratio=1)
