...
for (assay in assays) {
  name <- paste0("mutations_distances_annotation_median_rSASA_",assay,"_merged")
    dt <- get(name)
     ...
     # Loop through the unique values in the 'dataset' column and generate a plot for each
for (dataset_value in dataset_values) {
  # Subset the data for the current dataset value
  subset_dt <- dt[dataset == dataset_value]
  
  # Make a copy of the original table to avoid altering it in place
dt_new <- copy(subset_dt)

# Extract unique Pos and wt combinations
wt_rows <- unique(dt_new[, .(Pos, wt, dataset)])

#  Create the new rows with total_mean_kcal_mol = 0
wt_rows[, `:=`(
  mutant = wt,
  total_mean_kcal_mol = 0
)]
# Add colored label
...
rSASA_class_df[, wt_colored := paste0(
  ...
  pos_df<-subset_dt[,c("Pos","wt_Pos","wt","Pos_class_reduced","Pos_class")]
  ...
  pos_df <- merge(pos_df, rSASA_class_df[, .(Pos, wt_colored)], by = "Pos", all.x = TRUE)
     ...
     if (assay == "Folding") {#max and min of alll datasets
  min_val <- -3.06
  max_val <- 3.06
  
  p<-ggplot(dt_final,aes(x=Pos,y=mutant_colored))+
     geom_tile(aes(fill = total_mean_kcal_mol))+
   scale_fill_gradientn(colours = c("#898ac0","#f5f5f5","#e85d47"), values = rescale(c(min_val,0,max_val)),guide = "colorbar", limits=c(min_val,max_val))+
...
} else if (assay == "Binding") {#max and min of alll datasets
  min_val <- -1.53
  max_val <- 1.53
...
geom_jitter(aes(color=class), size = 1, alpha = 0.3,width = 0.005,height = 0.15)+
 ...
 geom_boxplot(width = 0.2, outlier.shape = NA, fill = NA, color = "black", size = 0.2) +
  scale_x_continuous(limits = c(-2, 3.1))+
     ...
     # Add one vertical line per dataset
    geom_vline(
      data = dt2,
